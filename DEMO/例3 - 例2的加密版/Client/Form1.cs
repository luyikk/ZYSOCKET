    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            if (SocketManager.client.ConnectionTo(RConfig.ReadString("Host"), RConfig.ReadInt("Port"))) //连接到服务器
            {
                ...
                SocketManager.client.Disconnection += new ExceptionDisconnection(client_Disconnection); //注册断开事件
                SocketManager.client.DataOn += new DataOn(client_DataOn);//注册数据包输入事件
                InitializeComponent();
